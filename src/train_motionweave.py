...
class MotionWeaveDataset(JointMetaWorldChunkDataset):
    """MetaWorld chunks paired with RoboEngine robot-arm masks."""

    def __init__(self, *args, roboengine_mask_root, **kwargs):
        super().__init__(*args, include_svd=False, **kwargs)
        self.roboengine_mask_root = Path(roboengine_mask_root)
        self._mask_cache = {}

    def _episode_mask_path(self, task_name, episode_idx):
        task_root = self.roboengine_mask_root / task_name
        candidates = (
...
class MotionWeavePolicy(ActionChunkInternVLPolicy):
    ...
    def build_motion_targets(self, motion_masks, num_visual_tokens):
        ...
        robot_occupancy = motion_masks.float().clamp(0.0, 1.0)
        batch_size = robot_occupancy.size(0)
        robot_occupancy = F.interpolate(
            robot_occupancy.reshape(
                -1,
                1,
                robot_occupancy.size(-2),
                robot_occupancy.size(-1),
            ),
            size=(grid_size, grid_size),
            mode="area",
        ).reshape(batch_size, self.action_horizon, -1)
        robot_occupancy = robot_occupancy + 1e-6
        return robot_occupancy / robot_occupancy.sum(dim=-1, keepdim=True)
...
def build_arg_parser():
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Root containing per-task robot-arm mask sequences generated by "
            "RoboEngine. --motion_mask_root is retained as a legacy alias."
        ),
    )
...
def train(args):
    ...
    try:
        ...
        dataset = MotionWeaveDataset(
            ...
            svd_image_size=128,
            roboengine_mask_root=args.roboengine_mask_root,
        )
        sampler = (
            DistributedSampler(
                dataset,
